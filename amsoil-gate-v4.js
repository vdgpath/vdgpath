(function () {
  console.log("AMSOIL Gate v4 Loaded");
  ...
  // Dev reset: add ?amsoil_reset=1 to the URL
  ...
  function shouldBypassGate() {
    ...
    if (localStorage.getItem(LEAD_CAPTURED_KEY) === "1") {
      console.log("AMSOIL Gate: lead already captured, bypassing");
      return true;
    }

    if (getDismissals() >= MAX_DISMISSALS) {
      console.log("AMSOIL Gate: dismissal limit reached, bypassing");
      return true;
    }
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (shouldBypassGate()) {
        console.log("AMSOIL Gate: allowing direct click");
        return;
      }
