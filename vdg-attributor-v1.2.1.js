...
function inferFieldFromElement(el) {
  ...
  function mergeFieldItems(primary = [], secondary = []) {
    ...
    [...primary, ...secondary].forEach((field) => {
      ...
      const key = `${field.name || ""}|${field.label || ""}|${valueString}`.toLowerCase();
      if (seen.has(key)) return;
      seen.add(key);
      merged.push(field);
    });

    return merged.slice(0, SC_MAX_FIELDS);
  }
  ...
  window.addEventListener("beforeunload", () => {
    ...
    // Prefer the most recent cached form snapshot; fallback to AJAX snapshot; fallback to message only.
    let fields = [];
    let message = scShadowMessage || null;

    try {
      const recentAjax = Array.from(scAjaxSnapshots.values()).sort((a, b) => b.cachedAt - a.cachedAt)[0];
      if (recentAjax && Date.now() - recentAjax.cachedAt < 20000) {
        fields = recentAjax.fields || [];
        message = recentAjax.bestMessage || message;
      }
    } catch {}
